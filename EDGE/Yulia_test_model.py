...
def plot_OG_Gen_graph(og_gen, OG_node_colors, OG_anomalous_edges,  g_gen, generated_node_colors, edge_labels, graph_probabilitiy, lowest_edge_probability, Monte_Carlo):
    fig, axes = plt.subplots(1, 2)
    pos = nx.circular_layout(og_gen)
    OG_edge_colors = ["red" if e in OG_anomalous_edges else "black" for e in og_gen.edges()]
...
def anomaly_confusion_matrix(true_graphs, per_graph_edge_list_counter, MC_counter) -> Tuple[int, int, int, int]:
    """Anomalous graph is one that has an anomalous edge/connection in it"""

    Original_anomaly_labels = []
    Generated_anomaly_labels = []
    for original_graph, generated_edges in zip(true_graphs, per_graph_edge_list_counter):
        original_edges = zip(*original_graph.edge_index.tolist())
        original_edges = sorted({tuple(sorted((u, v))) for u, v in original_edges if u != v})
        # if the original graph has an edge that was not generated by the model, or it was generated with probability < 0.1; then this edge is anomalous
        anomalous_edges_per_generated_graph = [edge for edge in original_edges if edge not in generated_edges.keys() or generated_edges[edge]/MC_counter < 0.1]
        
        if original_graph.anomalous.any():
            Original_anomaly_labels.append(1)
        else:
            Original_anomaly_labels.append(0)

        if len(anomalous_edges_per_generated_graph) > 0:
            Generated_anomaly_labels.append(1)
        else:
            Generated_anomaly_labels.append(0)


    tn, fp, fn, tp = confusion_matrix(Original_anomaly_labels, Generated_anomaly_labels).ravel()
...
if __name__ == '__main__':
    ...
    which_run = "./wandb/RelationalDataset_with_anomaly/multinomial_diffusion/multistep/2025-04-08_19-14-10" # 679

    path = which_run+"/check/checkpoint_679.pt"
    sample_numbers = np.arange(1000).tolist()
    Monte_Carlo = 100
    ...
    original_graphs, sampled_pygraph, per_graph_edge_list_counter, active_edges = model.sample_and_MC(sample_numbers, lambda_guidance = 1, MC = Monte_Carlo, noise_times=0) # 0 only conditioned, >0 subtracks the unconditioned, actively reducing the probability of generating samples that ignore the conditioning
    
    plot_active_edges_and_nodes(active_edges, sampled_pygraph.num_nodes)
    ...
    print("\ntn, fp, fn, tp : ",  anomaly_confusion_matrix(original_graphs, per_graph_edge_list_counter, Monte_Carlo))
    ...
    for sample_nb, (OG_graph, generated) in enumerate(zip(original_graphs, sampled_pygraph.to_data_list())):
        ...
        OG_anomalous_edges = [(u.item(), v.item()) for (u, v), is_anom in zip(zip(OG_graph.edge_index[0], OG_graph.edge_index[1]), OG_graph.edge_anomalous) if is_anom]
        ...
        if sample_nb < 2:
            print(f"this is graph number {sample_nb}")
            plot_OG_Gen_graph(og_gen, OG_node_colors, OG_anomalous_edges,  g_gen, generated_node_colors, edge_labels, graph_probabilitiy, lowest_edge_probability, Monte_Carlo)  
