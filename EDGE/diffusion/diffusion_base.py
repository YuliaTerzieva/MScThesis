...
class DiffusionBase(torch.nn.Module):
    ...
    def sample(self, num_samples): 
        _, batched_graph = self.initial_graph_sampler.sample(num_samples)
    ...
    def sample_and_MC(self, num_samples, lambda_guidance = torch.tensor(0), MC = 100, seed = 42, tuning = False): 
        ...
        for mc_counter in range(MC): # one loop takes on average 1.2 seconds
            ...
            for t in reversed(range(0, self.num_timesteps)): # one loop of this for takes between [0.07 to 0.22]s
            ...
            update_edge_occurence(batched_graph_mc_edge_index_and_count, working_clone) # this is very fast. around 0.04 s
