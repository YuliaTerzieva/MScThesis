...
np.random.seed(42)
training = pickle.load(open("GeneratedDataset_interm_graph/Synthetic_K7_edge_train", "rb"))
...
edge_embedding = torch.zeros((training_data.anomalous.shape[0], 2*node_attr_embedding.shape[1]))
for c, (i, j) in enumerate(training_data.edge_index.T): 
...
for run in np.arange(10): 
    ...
    iso_forest = IsolationForest(n_estimators=100, contamination=0.04, max_samples=256).fit(edge_embedding)
    
    testing = pickle.load(open("GeneratedDataset_interm_graph/Synthetic_K7_edge_test", "rb"))
    ...
    testing_edge_embedding = torch.zeros((testing_data.anomalous.shape[0], 2*testing_node_attr_embedding.shape[1]))
    for c, (i, j) in enumerate(testing_data.edge_index.T): 
        testing_edge_embedding[c] = torch.cat([testing_node_attr_embedding[i], testing_node_attr_embedding[j]])
    true_labels = testing_data.anomalous.detach().numpy().tolist()
    
    
    normality_score = iso_forest.decision_function(testing_edge_embedding)
    ...
    average_precision.append(average_precision_score(true_labels, abnormality_score_normed))
...
plt.yscale('log')
plt.legend()
plt.savefig(f"Plots/Plots_Isolation_Forest_EAD/{reduction_function}-EAD-ISOFOREST_score-dist")
plt.show()
...
plt.figure(figsize=(5,5))
plt.plot(interp_recall, mean_precision, label=f"Mean PR curve (AP = {round(np.mean(average_precision), 5)})", color = "#4A21A8")
...
plt.legend()
plt.ylim([0, 1.05])
plt.savefig(f"Plots/Plots_Isolation_Forest_EAD/{reduction_function}-EAD-ISOFOREST_AUPRC")
plt.show()
